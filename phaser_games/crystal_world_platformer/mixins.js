...
const collider = {
    addCollider(otherGameobj, callback, context) {
        this.scene.physics.add.collider(this, otherGameobj, callback, null, context || this)
        return this
    },

    addOverlap(otherGameobj, callback, context) {
        this.scene.physics.add.overlap(this, otherGameobj, callback, null, context || this)
        return this
    },
    ...
    raycast(body, layer, {raylength=30, precision=0, steepness=1}) {
        ...
        let condition1 = Math.abs(this.bodyPositionDifferenceX) <= precision;
        let condition2 = this.prevHasHit !== null;
        if(condition1 && condition2) return {ray: this.prevRay, hasHit: this.prevHasHit}
        ...
        if(hits.length > 0) hasHit = this.prevHasHit = hits.some(hit => hit.index !== -1);
